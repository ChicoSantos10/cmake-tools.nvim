...
function Config:update_test_target()
    local presets_exists = self.base_settings.use_preset and Presets.exists(self.cwd)
    if not presets_exists then
        return
    end
    ...
    if not self.test_preset then
        return
    end
    local test_preset = presets:get_test_preset(self.test_preset)
    if not test_preset then
        return
    end
    local test_target = test_preset:get_test_target()
