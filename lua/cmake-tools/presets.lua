...
function Presets:parse(cwd)
    ...
    local function createTestPreset(obj)
        return TestPreset:new(cwd, obj)
    end
    ...
    for _, test_preset in ipairs(instance.testPresets or {}) do
        test_preset = createTestPreset(test_preset)
    end
...
function Presets:get_test_preset_names(opts)
    ...
    local ret = {}
    for _, bpreset in ipairs(presets) do
        local cpresetName = self:get_test_preset(bpreset, opts).configurePreset
...
function Presets:get_test_preset(name, opts)
    return get_preset(name, self.testPresets, { include_hidden = true, include_disabled = true })
end
